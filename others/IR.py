...
def desExtra(features, codebook, desCate='BoF'):
    if desCate == 'BoF':
        ...
        # 新增VLAD实现
        if features.size == 0:
            return np.zeros(len(codebook) * codebook.shape[1])  # 初始化全零向量
        ...
        vlad /= (np.linalg.norm(vlad) + 1e-8)  # L2归一化
...
if __name__ == "__main__":
    ...
    def display_results(query_img, top_images, top_similarities):
        ...
        # 创建画布：3行5列（第一行1列，后两行各5列）
        fig, axes = plt.subplots(3, 5, figsize=(20, 10))  # 调整尺寸
